...
// Author2: Maya Ayman -> Added get_board member function 
// ID: 20210508
...
bool Board::is_winner() {
    ...
    for (int i:{0,1,2}) {
        if ( (row_win[i] && (row_win[i] == board[i][0]) && (board[i][0] != ' ')) ||
             (col_win[i] && (col_win[i] == board[0][i]) && (board[0][i] != ' ')) )
            {return true;}
    }
    if ((diag_win[0] && diag_win[0] == board[1][1] && board[1][1] != ' ') ||
